  ...
  async function onSubmit(values: z.infer<typeof resetPasswordSchema>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMessage = data.detail || data.message || data.error || 'Failed to reset password'
      ...
      <div className="flex min-h-screen items-center justify-center bg-background px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="w-full max-w-md space-y-8 rounded-lg border bg-card p-8 shadow-lg">
          <div className="text-center">
            <h2 className="mt-6 text-3xl font-bold tracking-tight text-foreground text-red-600">Invalid Reset Link</h2>
            <p className="mt-2 text-sm text-muted-foreground">
              This password reset link is invalid or has expired. Please request a new password reset.
            </p>
            <p className="mt-2 text-sm text-muted-foreground">
              Token: {token ? 'Present' : 'Missing'} | User ID: {userId ? 'Present' : 'Missing'}
            </p>
          </div>
