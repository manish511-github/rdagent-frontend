...
import {
  ...
  type CompetitorRow,
} from "@/components/kokonutui/competitors-table/columns";
...
async function fetchCompetitors(
  ...
  const res = await fetch(
    getApiUrl(`/company/competitor?user_id=${userId}&project_id=${projectId}`),
    { signal }
  );
...
async function postCompetitor(body: {
  ...
  if (res.status === 402) {
    ...
    const errorMessage =
      errorData?.detail || "Insufficient credits to create competitor.";
...
export default function CompetitorsPage({ projectId }: { projectId: string }) {
  ...
  const competitorsQuery = useQuery({
    ...
    queryFn: ({ signal }) =>
      fetchCompetitors(userId as number, resolvedProjectId as string, signal),
    refetchInterval: (data) =>
      Array.isArray(data) && data.some((c: any) => c.status === "running")
        ? 5000
        : false,
  });
  ...
  const existingDomains = useMemo(() => {
    ...
    (competitorsQuery.data as any[] | undefined)?.forEach((row) => {
      if (row?.competitor_source_id)
        set.add(sourceIdToDomain(row.competitor_source_id));
    });
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) =>
      toast({
        title: error?.message?.includes("Insufficient credits")
          ? "Insufficient Credits"
          : "Failed to create competitor",
        description:
          error?.message || "An error occurred while creating the competitor.",
        variant: "destructive",
      }),
  });
  ...
  const fetchDetailsMutation = useMutation({
    mutationFn: async (competitorUrl: string) => {
      ...
      if (response.status === 402) {
        ...
        const errorMessage =
          errorData?.detail || "Insufficient credits to analyze competitor.";
        toast({
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description:
          "We're analyzing the competitor. This may take a few minutes.",
      });
      // You can add navigation to the analysis page here if needed
    },
    onError: () =>
      toast({ title: "Failed to analyze competitor", variant: "destructive" }),
  });
  ...
  const refreshMutation = useMutation({
    mutationFn: async (competitorId: number) => {
      ...
      const competitor = competitorsQuery.data?.find(
      ...
      const competitorUrl = competitor.competitor_source_id
        ? `https://${competitor.competitor_source_id
            .replace(/_/g, ".")
            .replace(/\.com$/i, ".com")}/`
        : `https://${competitor.competitor_name
            .toLowerCase()
    ...
    onError: () =>
      toast({ title: "Failed to refresh competitor", variant: "destructive" }),
  });
  ...
  const { slug: previewSlug, hostname: previewHost } = useMemo(
    () => deriveSlugFromUrl(websiteUrl.trim() || ""),
    [websiteUrl]
  );
  ...
  useEffect(() => {
    ...
    setUrlError(
      validateUrl(websiteUrl) ? "" : "Enter a valid URL or domain (example.com)"
    );
  }, [websiteUrl]);

  function handleQuickAddSuggestion(s: {
    company_name: string;
    website: string;
    description: string;
  }) {
    ...
    const competitorUrl = s.website.startsWith("http")
      ? s.website
      : `https://${s.website}`;
    ...
    toast({
      ...
      description: `${s.company_name} added to your list and analysis started.`,
    });
      ...
      <div className="px-4 md:px-6 2xl:max-w-[1400px] mx-auto">
          ...
          <div className="w-full px-4 md:px-6 py-8 md:py-5 flex flex-col md:flex-row items-center gap-6">
            <div className="flex-1 space-y-2.5">
              ...
              <p className="text-slate-200 text-xs md:text-sm leading-relaxed max-w-xl font-normal">
                Boost your market strategy by adding a competitor via URL or
        ...
        <div className="mt-8">
          ...
          {competitorsQuery.isError && (
            <div className="text-sm text-destructive border-t py-4">
            ...
            filteredAndSorted.length === 0 && (
              <div className="col-span-full flex flex-col items-center justify-center py-12">
                ...
                <p className="text-sm text-muted-foreground text-center max-w-sm">
                  Add a competitor to start analyzing your market position and
                ...
                {view === "list" ? (
                  <div className="mt-2">
                    <DataTable
                      ...
                      data={filteredAndSorted.map<CompetitorRow>((c) => {
                        const apiItem = competitorsQuery.data?.find(
                          (apiItem: any) =>
                            apiItem.competitor_source_id ===
                            c.competitorSourceId
                        );
                        const website = apiItem?.competitor_source_id
                          ? `https://www.${apiItem.competitor_source_id
                              .replace(/_/g, ".")
                              .replace(/\.com$/i, ".com")}/`
                          : undefined;
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-2">
                    {filteredAndSorted.map((c, idx) => {
                      const apiItem = competitorsQuery.data?.find(
                        (apiItem: any) => apiItem.competitor_name === c.name
                      );
                      const sourceId = apiItem?.competitor_source_id as
                        | string
                        | undefined;
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Paste a website URL or choose from suggestions.
            </DialogDescription>
          </DialogHeader>
                      ...
                      <div className="flex items-start gap-3 min-w-0 flex-1">
                        <div className="min-w-0 space-y-1">
                          <div className="text-sm font-medium flex items-center gap-2 min-w-0">
                            ...
                            <span className="truncate" title={s.company_name}>
                              {s.company_name}
                            </span>
                          </div>
                          <div className="text-xs text-muted-foreground flex items-center gap-2 min-w-0">
                            <Globe className="h-3.5 w-3.5 flex-shrink-0" />
                            <span className="truncate" title={s.website}>
                              {s.website}
                            </span>
                          </div>
